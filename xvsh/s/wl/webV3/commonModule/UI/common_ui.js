var common_ui={
    ...
    initPop:function() {
        if($("#showPop").length==0){
            var div = '<div id="showPop" >' +
                '<div class="showContent">' +
                '<div class="showContTitl">' +
                '<div id="dialogCont"></div>' +
                '</div>' +
                '<div class="showContConf" >' +
                '<div class="cancelContent">取消</div>' +
                '<div class="confirmContent">确定</div>' +
                '</div>' +
                '</div>' +
                '</div>';
     ...
     *入参：
     * popObj
     * @popObj.dialogCont：弹框内容
     * @popObj.cancelContent:取消部分的名称，例如“取消”
     * @popObj.confirmContent：确定部分的名称，例如“我知道了”
     * @popObj.cancelback：点击取消之后回调方法
     * @popObj.confirmback：点击确定之后回调方法
     * ****/
    showPop:function(popObj){
        $("#dialogCont").html(popObj.dialogCont);
        if(cancelContent==undefined){
            $(".cancelContent").html("取消");
        }else{
            $(".cancelContent").html(popObj.cancelContent);
        }
        if(confirmContent==undefined){
            $(".confirmContent").html("确定");
        }else{
            $(".confirmContent").html(popObj.confirmContent);
        }
        $(".cancelContent").unbind('click').bind('click',function(){
            $("#showLog").hide();
            if(popObj.cancelback){
                popObj.cancelback();
            }
        });
        $(".confirmContent").unbind('click').bind('click',function(){
            $("#showLog").hide();
            if(popObj.confirmback){
                popObj.confirmback();
            }
        });
        $("#showLog").show();
