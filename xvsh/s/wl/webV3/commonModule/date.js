 ...
 * 滑动选取日期（年，月，日）
 * V1.1
 * 页面中需要创建一个id为datePlugin的空div
 * 页面需要加载date.css,即htmlads/css目录下的common.css文件
 */
(function ($) {
    $.fn.date = function (options, Ycallback, Ncallback) {
        //插件默认选项
        var that = $(this);
        var docType = $(this).is('span');
        var datetime = false;
        var nowdate = new Date($.ajax({
                type: "head",
                cache: false,
                async: false
            }).getResponseHeader("Date")) || new Date();
        var indexY = 1, indexM = 1, indexD = 1;
        var indexH = 1, indexI = 1, indexS = 0;
        var initY = parseInt((nowdate.getFullYear() + "").substr(0, 4)) - 2010;
        ...
        function init_iScrll() {
            ...
            var strY = parseInt($("#yearwrapper ul li:eq(" + indexY + ")").text());
            var strM = parseInt($("#monthwrapper ul li:eq(" + indexM + ")").text());

            yearScroll = new iScroll("yearwrapper", {
                snap: "li", vScrollbar: false,
                onScrollEnd: function () {
                    indexY = Math.round((this.y / 40) * (-1) + 1);
                    strY = parseInt($("#yearwrapper ul li:eq(" + indexY + ")").text());
            ...
            monthScroll = new iScroll("monthwrapper", {
                ...
                onScrollEnd: function () {
                    ...
                    strM = parseInt($("#monthwrapper ul li:eq(" + indexM + ")").text());
                    opts.endday = checkdays(strY, strM);
                    $("#daywrapper ul").html(createDAY_UL());
                    dayScroll.refresh();
                }
        ...
        function resetInitDete() {
            ...
            var _val = that.html();
            //initY = parseInt(_val.substr(3, 2))-15;
            //initM = parseInt(_val.substr(0, 2));
            //initD = parseInt(_val.substr(8, 2));
        }
        ...
        function bindButton() {
            ...
            $("#dateconfirm").unbind('click').click(function () {
                ...
                // +"-"+
                // $("#daywrapper ul li:eq("+Math.round(indexD)+")").html().substr(0,$("#daywrapper ul li:eq("+Math.round(indexD)+")").html().length);
                //alert($("#daywrapper ul li:eq("+indexD+")").html())
                //var datestr = $("#monthwrapper ul li:eq(" + (parseInt(indexM)) + ")").html() + "/" + $("#yearwrapper ul li:eq(" + (parseInt(indexY)) + ")").text().substr(2,2);
                if (datetime) {
